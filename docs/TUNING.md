...
- **Where the webhook points is the customer's to set.**
  `localmama.webhook_subscriptions` holds one active row per `agent_id`;
  `WEBHOOK_URL`/`WEBHOOK_SECRET` are only the fallback for an empty table. The
  secret is generated by the column default when a customer leaves it blank —
  "optional" in the dashboard means "we will make you one", never "unsigned".
  One active row is enforced by a unique index: the lead carries a single
  `handoff_status`, so fanning out to two endpoints would re-deliver to the one
  that already succeeded whenever the other failed.
