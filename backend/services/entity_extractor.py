...
def _looks_like_name(candidate: str) -> bool:
    ...
    if smalltalk.looks_like_chitchat(candidate):
        return False
    return True
...
def extract(text: str, expecting: Expecting | None = None) -> Extraction:
    ...
    name, name_conf = extract_name(text, expecting=expecting is Expecting.ASK_NAME)
    service, service_conf = extract_service(
        text, expecting=expecting is Expecting.ASK_SERVICE
    )
    location, location_conf = extract_location(
        text, expecting=expecting is Expecting.ASK_LOCATION
    )

    # A bare answer to one question must not bleed into the other fields.
    if expecting is Expecting.ASK_NAME and name_conf >= 0.6:
        if service_conf < 0.9:
            service, service_conf = None, 0.0
        if location_conf < 0.9:
            location, location_conf = None, 0.0
    if expecting is Expecting.ASK_LOCATION and location_conf >= 0.6:
        if name_conf < 0.9:
            name, name_conf = None, 0.0
    if expecting is Expecting.ASK_SERVICE and service_conf >= 0.6:
        if name_conf < 0.9:
            name, name_conf = None, 0.0

    # Language is never inferred here. The agent resolves it from what the
    # caller asked for and sends it as a `Language` value, because it has to —
    # it needs to know which language to speak next. Re-deriving it from a
    # service phrase would be guessing at something already known.
    language = None

    confidences = [c for c in (name_conf, service_conf, location_conf) if c > 0]
    return Extraction(
        name=name,
