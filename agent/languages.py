"""Working out which language the caller asked for.

The one piece of interpretation the agent still does, and it has to: it needs
to know which language to speak next, and no amount of asking the backend can
help with that inside a turn.

Everything else about a captured value — what it means, how it is spelled in
English, whether it matches anything — is the backend's.
"""
...
from contract import Language as _Language


#: Defined in the contract, not here. Both sides key on it — the agent to pick
#: a voice, the backend to pick a WhatsApp template — and two enums that must
#: agree are one enum with extra steps.
Language = _Language
